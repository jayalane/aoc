...
const data1 = `......124..................418.......587......770...........672.................564............................438..........512......653....
...
const (
	two  = 2  // test for 2 numbers adjacent
	five = 5  // default slice capacity for numbers in a line
	ten  = 10 // base ten
)
...
func getNumberAtPoint(i int64, j int64, d [][]string) number {
	beg := j
	end := j
	// find beginning of number
	for k := j; k >= 0; k-- {
		ok, _ := isDigitValue(d[i][k])
		if !ok {
			beg = k + 1
	...
	num := number{n, i, beg, i, end}

	return num
}

func anyNumbers(i int64, j int64, d [][]string) bool {
	if i >= 0 && j >= 0 && i < int64(len(d)) && j < int64(len(d[i])) {
		ch := d[i][j]
		isDigit, _ := isDigitValue(ch)
...
// checkStars returns true if the number is next to a non-period
// for now assuming in a row.
func checkStars(s star, d [][]string) (bool, []number) {
	numList := make([]number, 0, five)
	numCnt := 0
	...
	for i := int64(-1); i < 2; i++ {
		for j := int64(-1); j < 2; j++ {
			...
			// check corners
			ok := anyNumbers(s.row+i, s.col+j, d)
			if ok {
				num := getNumberAtPoint(s.row+i, s.col+j, d)
				found := false

				for _, oldOne := range numList {
					if oldOne.equal(num) {
						found = true
					}
				}
...
func getNums(l string, row int64, d *[][]string) ([]number, []star, error) { //nolint:unparam
	// first digit
	totals := make([]number, 0, five)
	stars := make([]star, 0, five)
	...
	// one last check for the final column for a pending number
	if startCol >= 0 {
		number := number{total, row, startCol, row, int64(len(l) - 1)}
		totals = append(totals, number)
	}

	return totals, stars, nil
}
...
func doLines(text string) error {
	...
	for row, l := range lines {
		...
		someNumbers, someStars, err := getNums(l, int64(row), &data)
		if err != nil {
	...
	for _, s := range stars {
		fmt.Println("Checking star", s)

		ratio := int64(1)

		ok, nums := checkStars(s, data)
		fmt.Println("Got ok, num", ok, nums)
	...
	fmt.Println("Total gear ratio", gearsTot)
