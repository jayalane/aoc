...
var errParseCard = errors.New("parse card failed")

func newParseCardError(msg string) error {
...
func parseCards(cards string) ([handSize]card, error) {
	parsedCards := [handSize]card{}
	if len(cards) != handSize {
		return parsedCards, newParseCardError("Len wrong for cards" + cards)
	}

	for i := 0; i < len(cards); i++ {
		cardID, ok := cardMapping[cards[i:i+1]]
		if !ok {
			return parsedCards, newParseCardError("Len value for cards" + cards)
		}

		parsedCards[i].value = cardID
	}
...
func handleLine(line string, hands *[]*hand) error {
	...
	bidNum, err := strconv.ParseUint(bid, 10, 64)
	if err != nil {
		return err
	}
	...
	parsedCards, err := parseCards(cards)
	if err != nil {
		return err
	}
